...
public class ForeOrderController extends BaseController {
    ...
    public String goToPage(HttpSession session, Map<String, Object> map,
        ...
        if (productOrderList.size() > 0) {
            orderCount = productOrderService.
                    getTotal(productOrder,
                            status_array);
                ...
                if (productOrderItemList != null) {
                    for (ProductOrderItem productOrderItem : productOrderItemList) {
                        ...
                        if (order.getProductOrderStatus() == 3) {
                            productOrderItem.setIsReview(reviewService.getTotalByOrderItemId(productOrderItem.getProductOrderItemId()) > 0);
                        }
                    }
                }
    ...
    public String goToOrderConfirmPage(@PathVariable("productId") Integer productId,
        ...
        map.put("orderTotalPrice", productOrderItem.getProductOrderItemPrice());
    ...
    public String goToOrderSuccessPage(Map<String, Object> map, HttpSession session,
        ...
        if (count == 1) {
            ...
            ProductOrderItem productOrderItem = productOrderItemService.getListByOrderId(order.getProductOrderId(), new PageUtil(0, 1)).get(0);
    ...
    public String updateOrderItem(HttpSession session, Map<String, Object> map, HttpServletResponse response,
        ...
        if (orderItemIDSet.size() > 0) {
            ...
            for (String key : orderItemIDSet) {
                ...
                Boolean yn = productOrderItemService.
                        update(productOrderItem1);
                if (!yn) {
    ...
    public String createOrderByOne(HttpSession session, Map<String, Object> map, HttpServletResponse response,
                                   ...
                                   @RequestParam Short orderItemNumber) throws UnsupportedEncodingException {
        JSONObject object = new JSONObject();
        logger.info("检查用户是否登录");
        Object userId = checkUser(session);
        if (userId == null) {
        ...
        Product product = productService.get(orderItemProductId);
        if (product == null) {
            object.put("success", false);
            object.put("url", "/");
            return object.toJSONString();
        ...
        ProductOrderItem productOrderItem = new ProductOrderItem();
        productOrderItem.setProductOrderItemUser(new User(Integer.valueOf(userId.toString())));
        productOrderItem.setProductOrderItemProduct(productService.get(orderItemProductId));
        productOrderItem.setProductOrderItemNumber(orderItemNumber);
        productOrderItem.setProductOrderItemPrice(product.getProductSalePrice() * orderItemNumber);
        productOrderItem.setProductOrderIte mUserMessage(userMessage);
        productOrderItem.setProductOrderItemOrder(new ProductOrder(orderId));

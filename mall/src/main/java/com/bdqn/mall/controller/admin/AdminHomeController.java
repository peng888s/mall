    ...
    private JSONObject getChartData(Date beginDate,Date endDate) throws ParseException {
        ...
        for (OrderGroup orderGroup : orderGroupList) {
            ...
            for (int j = 0; j < dateStr.length; j++) {
                if (dateStr[j].equals(orderGroup.getProductOrderPayDate())) {
                    index = j;
                }
            }
            switch (orderGroup.getProductOrderStatus()) {
                case 0:
                    orderUnpaidArray[index] = orderGroup.getProductOrderCount();
                    break;
                case 1:
                    orderNotShippedArray[index] = orderGroup.getProductOrderCount();
                    break;
                case 2:
                    orderUnconfirmedArray[index] = orderGroup.getProductOrderCount();
                    break;
                case 3:
                    orderSuccessArray[index] = orderGroup.getProductOrderCount();
                    break;
            }
        }
